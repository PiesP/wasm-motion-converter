...
const RUNTIME_DEP_VERSIONS = JSON.parse('__RUNTIME_DEP_VERSIONS__') as RuntimeDepVersions;

const getRuntimeDepVersion = (pkg: string): string => {
  const version = RUNTIME_DEP_VERSIONS[pkg];
  if (!version) {
    throw new Error(`[SW ${SW_VERSION}] Missing runtime dependency version for ${pkg}`);
  }
  return version;
};
...
async function verifyIntegrity(response: Response, expectedIntegrity: string): Promise<boolean> {
  try {
...
function classifyRequest(url: URL): RequestType {
  if (
    CDN_DOMAINS.some((domain) => url.hostname === domain || url.hostname.endsWith(`.${domain}`))
  ) {
    return 'cdn';
  }
...
function isWorkerRequest(request: Request): boolean {
  ...
  const workerPatterns = ['/worker.js', '/worker.mjs', 'ffmpeg/dist/esm/worker'];
        ...
        await Promise.allSettled(
          urls.map(async (url) => {
            try {
              ...
              console.warn(`[SW ${SW_VERSION}] CDN pre-cache failed: ${url}`, error);
            }
          })
        );
...
const CDN_CONVERTERS = {
  parseEsmSh(url: string): { pkg: string; version: string; path: string; isAsset: boolean } | null {
    try {
      const u = new URL(url);
  ...
  try {
    ...
    if (manifest) {
      ...
      if (expectedIntegrity) {
        ...
        if (!isValid) {
          console.error(`[SW ${SW_VERSION}] ✗ Integrity verification failed for ${provider.name}`);
          logCDNMetrics({
            url,
            cdnName: provider.name,
            latency,
      ...
      console.warn(`[SW ${SW_VERSION}] ⚠ SRI manifest not loaded, proceeding without verification`);
    }
...
function getAdaptiveTimeout(baseTimeout: number, connectionType: ConnectionType): number {
  ...
  try {
    ...
    for (const result of results) {
      if (result.status === 'fulfilled' && result.value.response) {
        ...
        console.log(`[SW ${SW_VERSION}] ✓ Parallel racing won by ${provider.name}`);
        return response;
      }
    }
...
async function fetchWithCascade(
  ...
  const shouldRace = useRacing !== undefined ? useRacing : connectionType === 'fast';
  ...
  if (shouldRace && orderedProviders.length >= 2) {
    console.log(`[SW ${SW_VERSION}] Racing ${orderedProviders.length} CDNs in parallel...`);
    ...
    console.warn(`[SW ${SW_VERSION}] Parallel racing failed, falling back to sequential cascade`);
  }
