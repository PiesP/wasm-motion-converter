...
describe('classifyConversionError', () => {
  ...
  describe('timeout errors', () => {
    ...
    it('classifies conversion timeout', () => {
      ...
      expect(result.type).toBe('timeout');
      expect(result.phase).toBe('ffmpeg_timeout');
  ...
  describe('codec errors', () => {
    it('classifies AV1+GIF combination', () => {
      ...
      const result = classifyConversionError('codec not supported', meta, undefined, ['[gif] encoding']);
    ...
    it('classifies AV1 decode failure', () => {
      ...
      const result = classifyConversionError('codec not supported', meta);
      expect(result.type).toBe('codec');
  ...
  describe('context fields', () => {
    ...
    it('includes ffmpeg logs when provided', () => {
      const logs = ['[info] Processing...', '[error] Timeout'];
      const result = classifyConversionError('timeout error', null, undefined, logs);
      expect(result.ffmpegLogs).toEqual(logs);
    });
  });
});
