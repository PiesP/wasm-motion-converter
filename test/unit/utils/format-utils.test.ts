...
import { formatDurationSeconds, formatBytes, createId } from '@utils/format-utils';
...
describe('formatDurationSeconds', () => {
  ...
  it('formats hours, minutes, and seconds', () => {
    ...
    expect(formatDurationSeconds(7322)).toBe('2:02:02');
  });

  it('handles negative input by clamping to 0', () => {
    expect(formatDurationSeconds(-5)).toBe('0:00');
    expect(formatDurationSeconds(-100)).toBe('0:00');
  });

  it('floors fractional seconds', () => {
    expect(formatDurationSeconds(5.9)).toBe('0:05');
    expect(formatDurationSeconds(61.1)).toBe('1:01');
  });
