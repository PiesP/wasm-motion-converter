...
latest_mature_npm_release() {
  local package_name="$1"
  local versions version published published_epoch

  versions="$(npm view "$package_name" time --json)"
  while IFS=$'\t' read -r version published; do
    ...
    jq -r 'to_entries | map(select(.key | test("^[0-9]+\\.[0-9]+\\.[0-9]+$"))) | sort_by(.value) | reverse[] | [.key, .value] | @tsv' <<< "$versions"
  )

  printf 'No stable %s release older than %s hours was found.\n' \
    "$package_name" "$cooling_hours" >&2
  return 1
}
...
check_npm_release() {
  ...
  expected="$(latest_mature_npm_release "$package_name")"
  if [[ "$current" != "$expected" ]]; then
...
codex_security_version="$(sed -nE 's/^[[:space:]]*CODEX_SECURITY_VERSION:[[:space:]]*"([^"[:space:]]+)".*/\1/p' .github/workflows/codex-security.yaml)"
