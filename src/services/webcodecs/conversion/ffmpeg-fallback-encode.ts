...
export async function encodeWithFFmpegFallback(params: {
  ...
  ffmpegService.reportProgress(
    FFMPEG_INTERNALS.PROGRESS.WEBCODECS.DECODE_START
  );
  ...
  try {
    fallbackResult = await decoder.decodeToFrames({
      ...
      targetFps,
      scale,
      frameFormat: FFMPEG_INTERNALS.WEBCODECS
        .FRAME_FORMAT as WebCodecsFrameFormat,
  ...
  const fallbackBlob = await ffmpegService.encodeFrameSequence({
    ...
    fps: fallbackFps,
    durationSeconds:
      fallbackDurationSeconds ?? metadata?.duration ?? fallbackResult.duration,
