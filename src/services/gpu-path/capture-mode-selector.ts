...
export class CaptureModeSelector {
  ...
  private detectCapabilities(): CaptureModeCapabilities {
    ...
    logger.info('capture-mode-selector', 'Detected capture mode capabilities', capabilities);
  ...
  selectMode(
    ...
    if (this.capabilities.demuxer && canUseDemuxer(file, metadata)) {
      ...
      if (isComplexCodec) {
        logger.info('capture-mode-selector', 'Selected demuxer mode for complex codec', {
          codec,
