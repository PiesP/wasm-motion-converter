...
const STRATEGY_MATRIX = new Map<string, CodecPathPreference>([
  ...
  [
    "h264:gif",
    {
      ...
      fallbackPath: "gpu",
      reason:
        "FFmpeg palettegen is faster than WebCodecs frame extraction for GIF",
      benchmarks: {
        avgTimeSeconds: 4.5,
        successRate: 0.98,
      },
  ...
  [
    "av1:gif",
    {
      ...
      reason:
        "AV1 prefers WebCodecs HW decode; falls back to FFmpeg CPU decode if unavailable",
      benchmarks: {
        avgTimeSeconds: 6.2,
        successRate: 0.89,
      },
  ...
  [
    "av1:mp4",
    {
      ...
      reason:
        "AV1 prefers WebAV; falls back to FFmpeg CPU transcode if unavailable",
      benchmarks: {
        avgTimeSeconds: 4.9,
        successRate: 0.91,
      },
  ...
  [
    "vp8:gif",
    {
      ...
      fallbackPath: "gpu",
      reason:
        "FFmpeg palettegen is typically faster than WebCodecs for VP8 GIF",
      benchmarks: {
        avgTimeSeconds: 5.0,
        successRate: 0.94,
      },
  ...
  [
    "vp9:gif",
    {
      ...
      fallbackPath: "gpu",
      reason:
        "FFmpeg palettegen is typically faster than WebCodecs for VP9 GIF",
      benchmarks: {
        avgTimeSeconds: 5.4,
        successRate: 0.91,
      },
...
class StrategyRegistryService {
  ...
  getStrategy(params: {
    ...
    if (
      ...
      recommended.confidence >=
        StrategyRegistryService.HISTORY_CONFIDENCE_THRESHOLD
    ) {
    ...
    return this.applyRecentFailureAvoidance(
      this.applyGifGpuPreference(
        this.applyHardwareDecodePreference({
          strategy: this.getHeuristicStrategy(
            normalizedCodec,
            format,
            capabilities
          ),
  ...
  private applyDurationHeuristics(
    ...
    if (
      (isHevcCodec(codec) || isAv1Codec(codec)) &&
  ...
  private applyRecentFailureAvoidance(
    ...
    const isFatalFfmpegFailure = (message?: string): boolean => {
      if (!message) return false;
      return (
        message.includes("RuntimeError: memory access out of bounds") ||
        message.includes("called FFmpeg.terminate()") ||
        message.includes("palette.png was not created")
      );
    };
    ...
    const hadRecentFatalCpuFailure = recentFatal.some(
      (r) =>
        r.path === "cpu" && !r.success && isFatalFfmpegFailure(r.errorMessage)
    );

    if (
      hadRecentFatalCpuFailure &&
      strategy.preferredPath === "cpu" &&
      strategy.fallbackPath !== strategy.preferredPath
    ) {
      logger.debug(
        "conversion",
        "Avoiding CPU due to recent fatal FFmpeg failure",
        {
      ...
      return {
        ...
        confidence:
          strategy.confidence === "high" ? "medium" : strategy.confidence,
        reason: `${strategy.reason} (avoiding recent fatal FFmpeg failure)`,
      };
    }
  ...
  getStrategyReasoning(params: {
    ...
    const containerSupport =
      decision === "gpu" ? !this.isBlockedContainer(container) : true;
    const hardwareAcceleration = capabilities.hardwareAccelerated;
    const codecHardwareDecodeHint = this.getCodecHardwareDecodeHint(
      normalizedCodec,
      capabilities
    );
    ...
    const gifGpuEligible =
      format === "gif"
        ? this.shouldPreferGpuForGif(normalizedCodec, capabilities)
        : undefined;
    const history = strategyHistoryService.getHistory(codec, format);
    ...
    if (!hasSupport) {
      if (capabilities.webcodecsDecode) {
        return {
          ...
          fallbackPath: "cpu",
          reason:
            "Unknown codec - trying GPU/WebCodecs first, with FFmpeg fallback",
          confidence: "low",
        };
      }
  ...
  private getRejectionReason(
    ...
    if (path === "gpu") {
      ...
      if (
        format === "gif" &&
        !this.shouldPreferGpuForGif(codec, capabilities)
      ) {
        const normalized = this.normalizeCodec(codec);
