...
export function transferBuffer(buffer: ArrayBuffer, transfer: Transferable[] = []): [ArrayBuffer, Transferable[]] {
...
export class AutoFrame implements Disposable {
  ...
  close(): void { this.frame.close(); }
  [Symbol.dispose](): void { this.close(); }
}

/** VideoFrame → ImageBitmap 변환 */
export async function frameToImageBitmap(frame: VideoFrame): Promise<ImageBitmap> {
  return createImageBitmap(frame, { resizeWidth: frame.displayWidth, resizeHeight: frame.displayHeight });
}
