...
function isProbablyMissingFileDeleteError(error: unknown): boolean {
  ...
  if (typeof error === "object" && error !== null) {
    ...
    const code =
      typeof maybeErr.code === "string" ? maybeErr.code.toLowerCase() : null;
    if (code === "enoent") {
      return true;
    }
...
export class FFmpegVFS {
  ...
  setInputCache(
    key: string,
    ttlMs: number = FFMPEG_INTERNALS.INPUT_CACHE_TTL_MS
  ): void {
  ...
  async deleteFile(ffmpeg: FFmpeg | null, fileName: string): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        "conversion",
        `Could not delete ${fileName} (non-critical)`,
        {
  ...
  async validateOutputFile(
    ...
    try {
      ...
      const validation = validateOutputBytes(
        new Uint8Array(data),
        expectedFormat
      );
      if (!validation.valid) {
  ...
  async handleConversionCleanup(
    ...
    try {
      ...
      if (isMemoryCritical()) {
        logger.debug(
          "conversion",
          "Cleanup: Memory critical - clearing cached input"
        );
        await this.clearCachedInput(ffmpeg);
      } else if (this.cachedInputKey) {
        logger.debug(
          "conversion",
          "Cleanup: Refreshing input cache with shorter TTL"
        );
        this.setInputCache(
          this.cachedInputKey,
          FFMPEG_INTERNALS.INPUT_CACHE_POST_CONVERT_MS
        );
      }
