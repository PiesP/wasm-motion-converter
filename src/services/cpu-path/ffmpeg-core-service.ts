...
export class FFmpegCore {
  ...
  private maxNonTerminalProgressSinceLastTerminal = 0;
   ...
   *
   * Note: FFmpeg.wasm's EventEmitter doesn't expose a way to remove all listeners.
   * This method is intentionally a no-op — individual conversion methods are responsible
   * for their own listener cleanup via off() calls in finally blocks.
   * The init log/progress handlers (initLogHandler, initProgressHandler) persist for
   * the lifetime of the FFmpeg instance and are set once during initialize().
   */
  clearAllListeners(): void {
    // Intentionally empty: FFmpeg.wasm does not expose removeEventListener for
    // its internal EventEmitter. Listeners are managed per-operation in encoder
    // finally blocks. See initLogHandler / initProgressHandler for init-time
    // handlers that live for the instance lifetime.
    logger.debug(
      'ffmpeg',
      'clearAllListeners called (no-op: FFmpeg.wasm has no global listener removal)'
    );
  }
  ...
  async initialize(callbacks?: InitializationCallbacks): Promise<void> {
    ...
    const setupFFmpegInstance = (): FFmpeg => {
      ...
      this.initProgressHandler = ({ progress }) => {
        ...
        const isInitializationProgress = this.isInitializing();

        // FFmpeg progress events do not reliably reset per exec() call. When a new operation starts
        // (often emitting 0%/1% again), allow the internal terminal-progress filters to reset.
        if (
        ...
        // consumers. In practice, this can also happen during short FFmpeg steps
        // like GIF palette generation, where only 0%/1% may be observed before a
        // spurious 100% emission.
        //
        // To keep logs and downstream consumers sane, ignore terminal-only 100%
        // emissions for conversion operations unless we've observed meaningful
        // non-terminal progress since the last terminal event.
        if (normalizedProgress === 100 && !isInitializationProgress) {
          if (
            this.maxNonTerminalProgressSinceLastTerminal <
            MIN_MEANINGFUL_NON_TERMINAL_PROGRESS_FOR_TERMINAL
          ) {
            return;
          }
          this.maxNonTerminalProgressSinceLastTerminal = 0;
        } else if (normalizedProgress < 100 && !isInitializationProgress) {
          this.maxNonTerminalProgressSinceLastTerminal = Math.max(
            this.maxNonTerminalProgressSinceLastTerminal,
            normalizedProgress
          );
        }
    ...
    const initializeWithVariant = async (coreVariant: 'mt' | 'st'): Promise<void> => {
      const variantLabel = coreVariant === 'mt' ? 'multithreaded' : 'single-threaded';
