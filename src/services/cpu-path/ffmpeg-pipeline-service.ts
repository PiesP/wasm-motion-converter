...
import { FFmpegCore } from '@services/cpu-path/ffmpeg-core-service';
import type { FFmpegInputOverride } from '@services/cpu-path/ffmpeg-encoder-service';
...
const FFMPEG_WASM_OUT_OF_BOUNDS_MESSAGE = 'memory access out of bounds';

function isFFmpegWasmOutOfBoundsError(error: unknown): boolean {
  const message = getErrorMessage(error).toLowerCase();

  if (message.includes(FFMPEG_WASM_OUT_OF_BOUNDS_MESSAGE)) {
    return true;
  }
