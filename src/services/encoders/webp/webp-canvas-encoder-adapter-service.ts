...
export class WebPCanvasEncoderAdapter implements EncoderAdapter {
  ...
  async encode(request: EncoderRequest): Promise<Blob> {
    ...
    try {
      ...
      const chunkSize = Math.min(MAX_CHUNK_SIZE, Math.max(MIN_CHUNK_SIZE, hwConcurrency * 2));
      ...
      const webpFrames: WebPFrame[] = encodedFrames.map((data, index) => ({
        ...
        duration: Math.max(
          1,
          durations[index] ?? Math.round(MAX_FRAME_DURATION_MS / Math.max(1, effectiveFps))
        ),
      }));
