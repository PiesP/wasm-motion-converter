...
const buildAssetUrl = (
  ...
  switch (provider.name) {
    case 'jsdelivr':
      return `${provider.baseUrl}/npm/${pkg}@${version}${cleanPath}`;
    default:
...
const WORKER_MODULE_MIME_TYPE = 'text/javascript';
const MODULE_IMPORT_REGEXES = [
  /import\s+[^'"\n]+\s+from\s+['"]([^'"]+)['"]/g,
  /import\s+['"]([^'"]+)['"]/g,
  /import\s*\(\s*['"]([^'"]+)['"]\s*\)/g,
];
...
const getBlobCompatibleProviders = () =>
  getEnabledProviders().filter(
    (provider) =>
      !PROVIDER_EXCLUSIONS.includes(provider.name as (typeof PROVIDER_EXCLUSIONS)[number])
  );

const logProviderExclusions = () => ({
  excludedProviders: [...PROVIDER_EXCLUSIONS],
...
const rewriteWorkerModuleImports = (source: string, baseUrl: string): WorkerModuleRewriteResult => {
  ...
  const recordRewrite = (original: string, resolved: string): void => {
    ...
    rewriteCount += 1;
    if (rewriteSamples.length < WORKER_IMPORT_REWRITE_SAMPLE_LIMIT) {
  ...
  for (const regex of MODULE_IMPORT_REGEXES) {
...
export async function loadFFmpegCoreAsset(
  ...
  const version = variant === 'mt' ? FFMPEG_CORE_VERSION : FFMPEG_CORE_ST_VERSION;
