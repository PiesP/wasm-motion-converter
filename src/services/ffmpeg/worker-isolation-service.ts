  ...
  try {
    ...
    const status = await withTimeout(
      new Promise<WorkerIsolationStatus>((resolve, reject) => {
        if (!worker) {
          reject(new Error(WORKER_CREATION_ERROR));
          return;
        }
        worker.onmessage = (event) => resolve(event.data as WorkerIsolationStatus);
        worker.onerror = () => reject(new Error(WORKER_CREATION_ERROR));
      }),
    ...
    if (!status.sharedArrayBuffer || !status.crossOriginIsolated) {
      throw new Error(WORKER_ISOLATION_ERROR);
    }
  } catch (error) {
    if (error instanceof Error) {
      if (
        ...
        throw new Error(WORKER_CONSTRUCTION_ERROR);
      }
    }
    throw error;
  } finally {
