...
const OPTIMAL_THREAD_RATIO = 0.75;
const MAX_THREADS = 12;
const MIN_SCALE_THREADS = 2;
const SCALE_THREAD_RATIO = 0.75;
...
function getOptimalThreadCount(): number {
  ...
  return Math.min(Math.floor(cores * OPTIMAL_THREAD_RATIO), MAX_THREADS);
}
    ...
    case 'scale-filter': {
      ...
      if (enableMultiThreadScale) {
        ...
        const threads = Math.max(
          MIN_SCALE_THREADS,
          Math.floor(getOptimalThreadCount() * SCALE_THREAD_RATIO)
        );
