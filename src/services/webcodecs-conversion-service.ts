    ...
    try {
        ...
        return await decoder.decodeToFrames({
          ...
          onFrame: async (frame) => {
            ...
            if (frame.data && frame.data.byteLength > 0) {
              ...
              logger.error(
                "conversion",
                `Rejected 0-byte frame from WebCodecs: ${frame.name}`,
                {
                  hasData: !!frame.data,
      ...
      if (orderedFrames.length > 0) {
        ...
        const calculateOptimalVfsBatchSize = (params: {
          ...
          const framesPerBudget = Math.floor(
            BatchMemoryBudget / estimatedFrameSize
          );
        ...
        for (let i = 0; i < orderedFrames.length; i += WriteBatchSize) {
          const batch = orderedFrames.slice(
            i,
            Math.min(i + WriteBatchSize, orderedFrames.length)
          );
        ...
        if (!cachedBatchSize) {
          ...
          logger.info(
            "conversion",
            "Cached VFS batch size for future conversions",
            {
              batchSize: WriteBatchSize,
            }
          );
      ...
      ffmpegService.reportStatus(
        `Converting frames to ${format.toUpperCase()}...`
      );
      const animationDurationSeconds = this.resolveAnimationDurationSeconds(
        frameFiles.length,
        effectiveTargetFps,
      ...
      logger.info(
        "conversion",
        "Using duration-aligned timestamps for complex codec WebP",
        {
          codec: metadata?.codec ?? "unknown",
          frameCount: frameFiles.length,
          durationSeconds: (
    ...
    try {
      if (this.shouldUseWebCodecsPath(metadata)) {
        ...
        try {
          ...
          logger.warn(
            "conversion",
            "WebCodecs direct path failed, trying FFmpeg fallback",
            {
              codec: metadata?.codec,
          ...
          logger.warn(
            "conversion",
            "WebCodecs direct path error, trying FFmpeg fallback",
            {
              error: errorMessage,
              codec: metadata?.codec,
          ...
          logger.debug(
            "conversion",
            "Continuing to FFmpeg fallback after WebCodecs error",
            {
            ...
            onFrame: async (frame) => {
              ...
              if (!frame.data || frame.data.byteLength === 0) {
                if (!lastValidEncodedFrame) {
                  logger.error(
                    "conversion",
                    "WebCodecs produced empty frame with no fallback",
                    {
