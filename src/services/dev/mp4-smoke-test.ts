...
export async function pickVideoFile(accept = 'video/*'): Promise<File> {
...
export function mountVideoPreview(url: string): HTMLVideoElement {
  ...
  const containerId = 'dropconvert-dev-mp4-smoke-test';
  ...
  if (!container) {
    ...
    title.textContent = 'MP4 Smoke Test (dev)';
...
export async function validateMp4Playback(
  ...
  try {
    ...
    try {
      await waitForEvent(video, 'canplay', 1_200);
    } catch {
      // ignore
    }
...
export async function runMp4SmokeTest(params?: {
  ...
  const playbackTimeoutMs = options.playbackTimeoutMs ?? 6_000;
