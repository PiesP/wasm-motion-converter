...
export class ProgressReporter {
  ...
  private static readonly DEFAULT_STATUS = 'Processing...';
  private static readonly PHASE_PROGRESS_MIN = 0;
  private static readonly PHASE_PROGRESS_MAX = 1;
  ...
  definePhase(name: string, start: number, end: number): void {
    if (
      start < ProgressReporter.MIN_PROGRESS ||
      start > ProgressReporter.MAX_PROGRESS ||
      end < ProgressReporter.MIN_PROGRESS ||
      end > ProgressReporter.MAX_PROGRESS
    ) {
  ...
  definePhases(phases: Array<{ name: string; weight: number }>): void {
    ...
    for (const phase of phases) {
      const range = (phase.weight / totalWeight) * ProgressReporter.MAX_PROGRESS;
      const start = currentStart;
      const end = Math.min(ProgressReporter.MAX_PROGRESS, currentStart + range);
  ...
  report(phaseProgress: number): void {
    ...
    const clampedProgress = Math.min(
      ProgressReporter.PHASE_PROGRESS_MAX,
      Math.max(ProgressReporter.PHASE_PROGRESS_MIN, phaseProgress)
    );
  ...
  reportAbsolute(progress: number): void {
    ...
    const clamped = Math.min(
      ProgressReporter.MAX_PROGRESS,
      Math.max(ProgressReporter.MIN_PROGRESS, progress)
    );
  ...
  complete(statusMessage?: string): void {
    this.reportAbsolute(ProgressReporter.MAX_PROGRESS);

    if (this.config.isActive()) {
      this.config.onStatus(statusMessage ?? ProgressReporter.DEFAULT_STATUS);
    }
  ...
  reset(): void {
    this.phases.clear();
    this.currentPhase = null;
    this.lastReportedProgress = ProgressReporter.MIN_PROGRESS;
