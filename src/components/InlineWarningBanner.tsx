...
const InlineWarningBanner: Component<InlineWarningBannerProps> = (props) => {
  ...
  const containerClasses = createMemo(() => {
    switch (highestSeverity()) {
      case 'critical':
        return 'bg-red-50 dark:bg-red-950 border-l-4 border-red-400 dark:border-red-500 rounded-lg p-4';
      case 'high':
        return 'bg-orange-50 dark:bg-orange-950 border-l-4 border-orange-400 dark:border-orange-500 rounded-lg p-4';
      default:
        return 'bg-yellow-50 dark:bg-yellow-950 border-l-4 border-yellow-400 dark:border-yellow-500 rounded-lg p-4';
    }
  });
  ...
  const iconClasses = createMemo(() => {
    switch (highestSeverity()) {
      case 'critical':
        return 'h-5 w-5 text-red-400 dark:text-red-500';
      case 'high':
        return 'h-5 w-5 text-orange-400 dark:text-orange-500';
      default:
        return 'h-5 w-5 text-yellow-400 dark:text-yellow-500';
    }
  });

  const titleClasses = createMemo(() => {
    switch (highestSeverity()) {
      case 'critical':
        return 'text-sm font-medium text-red-800 dark:text-red-300';
      case 'high':
        return 'text-sm font-medium text-orange-800 dark:text-orange-300';
      default:
  ...
  const textClasses = createMemo(() => {
    switch (highestSeverity()) {
      ...
      case 'high':
        return 'mt-2 text-sm text-orange-700 dark:text-orange-400';
      default:
        return 'mt-2 text-sm text-yellow-700 dark:text-yellow-400';
    }
  });
  ...
  const buttonClasses = createMemo(() => {
    switch (highestSeverity()) {
      case 'critical':
        return 'inline-flex items-center px-3 py-2 border border-transparent text-xs font-medium rounded-md text-red-900 bg-red-200 hover:bg-red-300 dark:text-red-100 dark:bg-red-800 dark:hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-gray-900';
      case 'high':
        return 'inline-flex items-center px-3 py-2 border border-transparent text-xs font-medium rounded-md text-orange-900 bg-orange-200 hover:bg-orange-300 dark:text-orange-100 dark:bg-orange-800 dark:hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 dark:focus:ring-offset-gray-900';
      default:
      ...
      <div class="flex items-start">
        ...
        <div class="ml-3 flex-1">
          <div class="flex items-center gap-2">
            <h3 class={titleClasses()}>{titleText()}</h3>
          ...
          <div class={textClasses()}>
            ...
            <Show when={props.onAction && props.actionLabel}>
              <div class="mt-3">
                <button type="button" class={buttonClasses()} onClick={props.onAction}>
                  {props.actionLabel}
                </button>
              </div>
            </Show>
